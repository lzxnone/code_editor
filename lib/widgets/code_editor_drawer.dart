...
class CodeEditorDrawer extends StatelessWidget {
  ...
  Future<void> _handleNewFile(BuildContext context, String rootPath) async {
    ...
    if (name != null && name.trim().isNotEmpty && context.mounted) {
      try {
        ...
        if (context.mounted) {
          DialogUtils.showErrorToast(context, l10n?.createFileFailed(e.toString()) ?? '创建文件失败: $e');
        }
      }
    }
  }
  ...
  Future<void> _handleNewFolder(BuildContext context, String rootPath) async {
    ...
    if (name != null && name.trim().isNotEmpty && context.mounted) {
      try {
        await _getProjectProvider(context).createDirectory(rootPath, name.trim());
      } catch (e) {
        if (context.mounted) {
          DialogUtils.showErrorToast(context, l10n?.createFolderFailed(e.toString()) ?? '创建文件夹失败: $e');
        }
      }
    }
  }
                    ...
                    children: [
                      ...
                      if (hasProject) ...[
                        ...
                        IconButton(
                          ...
                          onPressed: () => _handleNewFolder(context, rootPath),
                        ),
                      ],
                    ],
                  ),
