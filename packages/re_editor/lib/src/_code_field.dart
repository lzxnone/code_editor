...
class _CodeFieldRender extends RenderBox implements MouseTrackerAnnotation {
  ...
  DateTime? _vDwellStartTime;
  DateTime? _hDwellStartTime;
  ...
  /// Calculates auto-scroll velocity (pixels/sec) using a high-acceleration dual-zone model:
  /// - Inner buffer zone (0 to edgeThreshold from edge): linear crawl from [minVelocity] to [edgeVelocity].
  /// - Outer pull zone (beyond edge into overscroll): rapid acceleration over a compact distance ([maxPullDistance]).
  ...
  void autoScrollWhenDragging(Offset dragPosition) {
    ...
    final double vEdgeThreshold = max(32.0, lineHeight * 1.5);
    const double vMaxPullDistance = 45.0;

    // 水平方向针对移动端狭窄屏幕优化：
    // 1. 更宽的内侧触控带 (40px)，无需紧贴屏幕物理边缘
    // 2. 超紧凑外拉加速距离 (15px)，碰到边距或边框即可快速起速
    // 3. 驻留自适应加速 (Dwell Acceleration)，按住边缘原地不动即可持续加速
    final double hEdgeThreshold = max(40.0, lineHeight * 1.8);
    const double hMaxPullDistance = 15.0;
    ...
    bool vActive = false;
    if (_verticalViewportSize != null && _verticalViewportSize! > 0) {
      if (offset.dy < vEdgeThreshold && _verticalViewport.pixels > 0) {
        vActive = true;
        _vDwellStartTime ??= now;
        final double dwellSeconds = now.difference(_vDwellStartTime!).inMicroseconds / 1000000.0;
        ...
        if (v > 0) {
          ...
          final double target = max(0.0, _verticalViewport.pixels - delta);
          if (target != _verticalViewport.pixels) {
            _verticalViewport.jumpTo(target);
            scrolled = true;
          }
        }
      } else if (offset.dy > size.height - vEdgeThreshold && _verticalViewport.pixels < _verticalViewportSize!) {
        vActive = true;
        _vDwellStartTime ??= now;
        final double dwellSeconds = now.difference(_vDwellStartTime!).inMicroseconds / 1000000.0;
        final double dwellMultiplier = dwellSeconds > 0.25
        ...
        if (v > 0) {
          final double delta = v * dt;
          final double target = min(_verticalViewportSize!, _verticalViewport.pixels + delta);
          if (target != _verticalViewport.pixels) {
            _verticalViewport.jumpTo(target);
            scrolled = true;
          }
    ...
    if (!vActive) {
      _vDwellStartTime = null;
    }

    // Horizontal auto-scroll
    bool hActive = false;
    if (_horizontalViewport != null && _horizontalViewportSize != null && _horizontalViewportSize! > 0) {
      if (offset.dx < hEdgeThreshold && _horizontalViewport!.pixels > 0) {
        hActive = true;
        _hDwellStartTime ??= now;
        final double dwellSeconds = now.difference(_hDwellStartTime!).inMicroseconds / 1000000.0;
        final double dwellMultiplier = dwellSeconds > 0.2
            ? 1.0 + ((dwellSeconds - 0.2) / 0.7).clamp(0.0, 1.0) * 3.5
            : 1.0;
        final double baseV = _calculateAutoScrollVelocity(
          pointerPosition: offset.dx,
          viewportDimension: size.width,
          edgeThreshold: hEdgeThreshold,
          isStartEdge: true,
          minVelocity: 70.0,
          edgeVelocity: 200.0,
          maxVelocity: 1300.0,
          maxPullDistance: hMaxPullDistance,
        );
        final double v = min(1400.0, baseV * dwellMultiplier);
        if (v > 0) {
          final double delta = v * dt;
          final double target = max(0.0, _horizontalViewport!.pixels - delta);
          if (target != _horizontalViewport!.pixels) {
            _horizontalViewport!.jumpTo(target);
            scrolled = true;
          }
        }
      } else if (offset.dx > size.width - hEdgeThreshold && _horizontalViewport!.pixels < _horizontalViewportSize!) {
        hActive = true;
        _hDwellStartTime ??= now;
        final double dwellSeconds = now.difference(_hDwellStartTime!).inMicroseconds / 1000000.0;
        final double dwellMultiplier = dwellSeconds > 0.2
        ...
        if (v > 0) {
          final double delta = v * dt;
          final double target = min(_horizontalViewportSize!, _horizontalViewport!.pixels + delta);
          if (target != _horizontalViewport!.pixels) {
            _horizontalViewport!.jumpTo(target);
            scrolled = true;
          }
  ...
  /// Calculates the bounding rectangle (in global screen coordinates) of the given selection.
  Rect? calculateSelectionBoundingRect(CodeLineSelection selection) {
    if (selection.isCollapsed) {
      final Offset? offset = calculateTextPositionScreenOffset(selection.start, false);
      if (offset == null) return null;
      return Rect.fromLTWH(offset.dx, offset.dy, 0, _preferredLineHeight);
    }
    final List<CodeLineRenderParagraph> paragraphs = _displayParagraphs;
    ...
    for (final CodeLineRenderParagraph paragraph in paragraphs) {
      ...
      final Offset paragraphGlobalOffset = localToGlobal(paragraph.offset - paintOffset);
      for (final Rect r in rects) {
        final Rect globalR = r.shift(paragraphGlobalOffset);
        if (globalR.left < minX) minX = globalR.left;
        if (globalR.top < minY) minY = globalR.top;
        if (globalR.right > maxX) maxX = globalR.right;
        if (globalR.bottom > maxY) maxY = globalR.bottom;
        foundAny = true;
      }
    }
