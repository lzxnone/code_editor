...
class IndexAwareCircularBuffer<T extends IndexedItem> {
  ...
  /// [index] to the left.
  ///
  /// === code_editor 本地补丁（上游 4.0.0 缺陷）===
  ///
  /// 上游原实现只调整 `_startIndex` / `_length`：
  ///
  /// ```dart
  ...
  /// 这会造成两个后果：
  ///
  /// 1. `IndexedItem.index`（= `_absoluteIndex - _absoluteStartIndex`）不再等于元素在
  ///    列表中的位置，而是整体偏移“被裁掉的行数”。`BufferLine.index` 正是
  ///    `CellAnchor.y` / `CellAnchor.offset` 的来源，于是锚点与 `lines[y]` 的坐标系
  ///    自相矛盾：终端选区无法绘制、`Buffer.getText(range)` 读出空串或别的行。
  /// 2. 被裁掉的元素从不 `_detach()`，仍然 `attached == true` 且留在 `_array` 里，
  ///    旧锚点既不失效又会静默指向其它行（“僵尸锚点”）。
  ///
  /// 触发链：`ESC[3J` → `Buffer.clearScrollback()` → `trimStart()`，即 shell 的
  /// `clear` / `Ctrl+L`（xterm-256color 带 `E3` 能力）之后终端框选失效。
  ///
  /// 修复方式：委托给上游自身已验证的 `remove(index, count)` 路径 —— 它会
  /// `_detach()` 被移除的元素、并用 `_move()` 重新校准存活元素的绝对索引，
  /// 两个后果一并消除。代价是把“只调窗口”变成 O(存活元素) 的搬移，
  /// 而调用场景（清空回滚缓冲）是低频操作。
  void trimStart(int count) {
    if (count > _length) count = _length;
    if (count <= 0) return;
